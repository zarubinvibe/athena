...
Athena 用版本化的模板和检查，在一台新的 Mac 上重建完整的 Claude Code 与 Codex 工作环境。
...
Athena 是一套可携带的 macOS 智能体操作系统。它把你的智能体配置保存成版本化的模板、清单和检查，然后在新机器上一层一层重建。规则、技能、项目和知识都回到原来的位置。

## 它解决什么问题

一套好用的智能体环境要长好几个月：配置、技能、钩子、注册表、克隆好的项目。换一台笔记本或者账户损坏，全没了。Athena 让这套环境变成一个晚上就能重建的东西，而不是靠脑子回忆。

## 最大的优势

**最大的优势：** 环境是一段可以重复执行的程序，而不是记忆。

**为什么这样更好：** 每一层都可以再跑一次而不破坏已有的东西，`--dry-run` 会在写下第一个文件之前把整个计划打出来。

## 工作流程

引导脚本按顺序执行分层。你可以先预览，可以只跑其中一层，也可以跑完整条链。
...
Athena 面向 macOS。需要管理员账户，以及 Homebrew、npm 和 Git 的网络访问。
...
状态：面向 macOS 的公开参考实现。自动检查覆盖 shell 语法、模板、卫生、智能体契约和洁净渲染。
...
想看得更深：[完整参考](docs/DETAILS.md)、[功能说明](docs/FEATURES.en.md)、[文件系统契约](rules/structure.md)、[路线图](specs/00-roadmap.md) 和 [真机验收](smoke/live-acceptance.md)。
