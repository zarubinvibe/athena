...
Athena это переносимая агентная ОС для macOS. Она держит вашу настройку в виде версионных шаблонов, манифестов и проверок, а потом слой за слоем собирает её на новой машине. Правила, скиллы, проекты и знания возвращаются на свои места.

## Зачем это нужно

Рабочая агентная настройка растёт месяцами: конфиги, скиллы, хуки, реестры, клонированные проекты. Новый ноутбук или сломанный профиль стирает всё. Athena превращает эту настройку в то, что собирается за вечер, а не вспоминается руками.

## Главное преимущество

**Главное преимущество:** настройка это повторяемая программа, а не память.

**Почему это лучше:** любой слой можно прогнать повторно, не ломая уже собранное, а `--dry-run` показывает весь план до записи первого файла.

## Как это работает

Bootstrap идёт упорядоченными слоями. Их можно посмотреть заранее, запустить один кусок или прогнать всю цепочку.
...
Athena рассчитана на macOS. Нужны учётная запись администратора и сеть для Homebrew, npm и Git.
...
Прочитайте [SECURITY.md](SECURITY.md), прежде чем запускать Athena на машине с важными данными.

## Ограничения

Статус: публичная эталонная реализация для macOS. Автопроверки закрывают синтаксис shell, шаблоны, гигиену, агентские контракты и чистый рендер.
...
Глубже: [полный разбор](docs/DETAILS.ru.md), [описание возможностей](docs/FEATURES.en.md), [контракт файловой системы](rules/structure.md), [дорожная карта](specs/00-roadmap.md) и [живая приёмка](smoke/live-acceptance.md).
